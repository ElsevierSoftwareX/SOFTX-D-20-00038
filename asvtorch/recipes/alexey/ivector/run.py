...
import asvtorch.recipes.alexey.data_preparation as data_preparation  # same preparation as in deep embedding recipe
...
class RecipeSettings(AbstractSettings):
    ...
    crop_duration: int = None  # Use full segments
...
for settings_string in Settings().load_settings(run_config_file, run_configs):
    ...
    if Settings().recipe.start_stage <= 4 <= Settings().recipe.end_stage:
        ...
        training_data = UtteranceSelector().choose_all('voxceleb2')
    ...
    if Settings().recipe.start_stage <= 10 <= Settings().recipe.end_stage:

        output_folder = '/media/hdd2/vvestman/alexey_embeddings/ivectors'
        ivector_extractor = IVectorExtractor.from_npz(Settings().recipe.selected_iteration, Settings().computing.device)

        def compute_dataset_posteriors_and_ivectors(dataset, output_folder):
            crop_duration = Settings().recipe.crop_duration
            print('Loading {} data...'.format(dataset))
            data = UtteranceSelector().choose_all(dataset)
            if crop_duration:
                data.clip_utterances_to_length(crop_duration, crop_duration)
            else:
                crop_duration = 'full'
            print('Extracting {} embeddings...'.format(dataset))
            extract_posteriors(data)
            #data.save('posteriors_{}_{}'.format(dataset, crop_duration), output_folder)

            ivector_extractor.extract(data)
            data.save('ivectors_{}_{}'.format(dataset, crop_duration), output_folder)
            data.export('ivectors_{}_{}'.format(dataset, crop_duration), output_folder)

        compute_dataset_posteriors_and_ivectors('librispeech', output_folder)
        compute_dataset_posteriors_and_ivectors('voxceleb1', output_folder)
        compute_dataset_posteriors_and_ivectors('voxceleb2', output_folder)
